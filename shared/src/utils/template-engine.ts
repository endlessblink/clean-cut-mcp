...
import { ANIMATION_CATEGORIES, ANIMATION_PATTERNS } from '../config/constants';
...
export class TemplateEngine {
  ...
  private generateBouncingBallCode(): string {
    return `
...
`.trim();
  ...
  private generateSlidingTextCode(): string {
    return `
...
`.trim();
  ...
  private generateRotatingLogoCode(): string {
    return `
import { interpolate, spring } from 'remotion';
import { useCurrentFrame, useVideoConfig } from 'remotion';
...
`.trim();
  ...
  private generateFadeInOutCode(): string {
    return `
import { interpolate } from 'remotion';
import { useCurrentFrame, useVideoConfig } from 'remotion';
