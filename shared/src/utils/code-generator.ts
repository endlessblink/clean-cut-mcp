import { CreateAnimationRequest, ValidationResult } from '../types/animation';
import { enforceBaseRules } from '../validation/base-animation-rules';
...
export class CodeGenerator {
  ...
  generateFromDescription(description: string, options: {
    ...
    if (lowerDesc.includes('bounce') || lowerDesc.includes('ball')) {
      return templateEngine.generateCode('bouncing-ball', options.props) || this.generateBasicBounce(options);
    }

    if (lowerDesc.includes('slide') || lowerDesc.includes('text')) {
      return templateEngine.generateCode('sliding-text', {
    ...
    if (lowerDesc.includes('rotate') || lowerDesc.includes('logo') || lowerDesc.includes('spin')) {
      return templateEngine.generateCode('rotating-logo', options.props) || this.generateBasicRotate(options);
    }

    if (lowerDesc.includes('fade') || lowerDesc.includes('opacity')) {
      return templateEngine.generateCode('fade-in-out', options.props) || this.generateBasicFade(options);
    }
   ...
   */
  private generateBasicBounce(options: any): string {
...
`.trim();
   ...
   */
  private generateBasicSlide(options: any): string {
...
`.trim();
   ...
   */
  private generateBasicRotate(options: any): string {
...
`.trim();
   ...
   */
  private generateBasicFade(options: any): string {
...
`.trim();
  ...
  validateCode(code: string): ValidationResult {
    ...
    const baseValidation = enforceBaseRules({
