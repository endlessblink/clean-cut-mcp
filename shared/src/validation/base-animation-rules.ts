 ...
 */
export function enforceBaseRules(spec: AnimationSpec): ValidationResult {
  ...
  const hasElementScale = spec.scenes?.some(s =>
    s.elements?.some(e => e.scale && e.scale !== 1.0)
  );
  if (hasElementScale) {
  ...
  const missingBlur = spec.scenes?.some(s =>
    s.elements?.some(e => (e.velocity && e.velocity > 3) && !s.hasMotionBlur)
  );
  if (missingBlur) {
