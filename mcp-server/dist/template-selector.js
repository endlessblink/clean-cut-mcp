 ...
 */
export function analyzeUserRequest(prompt) {
...
function detectPlatform(prompt) {
    if (/\b(youtube|yt)\b/i.test(prompt))
        return 'youtube';
    if (/\b(instagram|insta|ig)\b/i.test(prompt))
        return 'instagram';
    if (/\b(tiktok|tik tok)\b/i.test(prompt))
        return 'tiktok';
    if (/\b(linkedin)\b/i.test(prompt))
        return 'linkedin';
    if (/\b(twitter|x\.com)\b/i.test(prompt))
        return 'twitter';
    if (/\b(facebook|fb)\b/i.test(prompt))
        return 'facebook';
    return undefined;
}
/**
 ...
 */
function detectAspectRatio(prompt, platform) {
    // Explicit mentions
    if (/\b(vertical|portrait|9:16|story|reel|short)\b/i.test(prompt))
        return '9:16';
    if (/\b(square|1:1)\b/i.test(prompt))
        return '1:1';
    if (/\b(horizontal|landscape|16:9)\b/i.test(prompt))
        return '16:9';
    if (/\b(4:5)\b/i.test(prompt))
        return '4:5';
    // Platform defaults
...
function detectEnergy(prompt) {
    ...
    if (highEnergy.test(prompt))
        return 8;
    if (lowEnergy.test(prompt))
        return 3;
 ...
 */
function detectProfessionalism(prompt) {
    ...
    if (corporate.test(prompt))
        return 9;
    if (casual.test(prompt))
        return 3;
 ...
 */
export function selectTemplates(templates, userPrompt, topN = 3) {
    ...
    const scored = templates.map(template => {
        ...
        const matchedKeywords = template.keywords.filter(tk => userRequest.keywords.some(uk => uk.includes(tk) || tk.includes(uk)));
        // Generate reason
 ...
 */
function generateMatchReason(template, userRequest, score, matchedKeywords) {
    const reasons = [];
    // Keyword matches
 ...
 */
function generateWarnings(template, userRequest) {
    const warnings = [];
