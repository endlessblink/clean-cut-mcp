...
No more "Cannot find module" errors!`
    ...
    async handleListQualityPresets(args) {
...
💡 Use 'get_quality_for_use_case' to get the recommended preset for your specific needs.
   Use 'configure_render_quality' to apply a preset to your Remotion configuration.`
    ...
    async handleGetQualityPreset(args) {
...
⚙️ Use 'configure_render_quality' with presetName: "${presetName}" to apply these settings.`
    ...
    async handleGetQualityForUseCase(args) {
        const { useCase } = args;
        try {
...
⚙️ Apply this preset with: configure_render_quality { presetName: "${Object.keys(QUALITY_PRESETS).find(k => QUALITY_PRESETS[k] === preset) || useCase}" }`
    ...
    async handleConfigureRenderQuality(args) {
        ...
        try {
            ...
            if (customSettings) {
                if (customSettings.bitrate)
                    finalSettings.bitrate = customSettings.bitrate;
                if (customSettings.crf !== undefined)
                    finalSettings.crf = customSettings.crf;
                if (customSettings.resolution) {
                    finalSettings.resolution = { ...finalSettings.resolution, ...customSettings.resolution };
                }
                if (customSettings.frameRate)
                    finalSettings.frameRate = customSettings.frameRate;
            }
...
const currentConfig = {
  ...
  fps: ${finalSettings.frameRate}
            ...
            if (currentConfig.includes('// DYNAMIC QUALITY CONFIGURATION:')) {
                ...
                updatedConfig = currentConfig.replace(/\/\/ DYNAMIC QUALITY CONFIGURATION:[\s\S]*?const currentConfig = {[\s\S]*?};/, newQualitySettings.trim());
            }
            else {
                // Append before the last line
                updatedConfig = currentConfig.replace(/\/\/ Font support enabled via Dockerfile: fonts-noto-color-emoji \+ fontconfig\s*$/, newQualitySettings.trim() + '\n\n// Font support enabled via Dockerfile: fonts-noto-color-emoji + fontconfig');
            }
