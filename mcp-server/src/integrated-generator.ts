...
import { applyLearnedRules } from './preference-applier';
import { enforceAllRules, generateEnforcementReport, AnimationSpec, SceneSpec } from './rule-enforcer';
...
export interface GenerationResult {
  ...
  code: string;
  metadata: {
    duration: { total_frames: number; total_seconds: number; formula: string };
    analysis: any;
    brand: any;
    appliedRules: string[];
    enforcement: any;
 ...
 */
export async function generateAnimation(request: GenerationRequest): Promise<GenerationResult> {
  ...
  if (!enforcement.valid) {
    ...
    throw new Error(
      `Animation violates ${enforcement.mustFix.length} learned rules. Fix violations before generating.`
    );
  }
