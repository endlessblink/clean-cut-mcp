...
export interface QualityPreset {
  ...
  audioBitrate: string;
  useCase: string;
  estimatedFileSize: string; // per minute of video
}
...
export function listQualityPresets(): Array<{ key: string; preset: QualityPreset }> {
  return Object.entries(QUALITY_PRESETS).map(([key, preset]) => ({ key, preset }));
}

export function getQualityForUseCase(useCase: 'presentation' | 'web' | 'cinema' | 'social' | 'demo'): QualityPreset {
  switch (useCase) {
    case 'presentation':
...
export function generateRenderConfig(preset: QualityPreset): {
  codec: string;
  bitrate: string;
  crf: number;
  pixelFormat: string;
