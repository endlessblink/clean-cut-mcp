...
class CleanCutMcpServer {
  ...
  private setupTools() {
    ...
    this.server.tool('create_animation', {
      ...
      try {
        ...
        const animationTitle = title || type.split('-').map(word => 
  ...
  private generateAnimationComponent(type: string, componentName: string, backgroundColor: string, displayTitle?: string): string {
    const components = {
...
${displayTitle || componentName}
  ...
  private async updateRootTsx(componentName: string, filename: string): Promise<void> {
