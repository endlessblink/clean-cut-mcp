...
### **Motion Pattern Settings:**
```
TRANSITION_STYLE: smooth         (smooth / cinematic / dynamic)
CAMERA_MOVEMENT: disabled        (enabled / disabled)
PARALLAX_EFFECT: enabled         (enabled / disabled)
MORPH_QUALITY: standard          (standard / high-precision)
MASK_ANIMATION: simple           (simple / complex)
```

### **Advanced Pattern Options:**
```
USE_CUT_ON_ACTION: false         (Seamless action continuity across cuts)
USE_MATCH_CUTS: false            (Visual similarity transitions)
USE_DOLLY_MOVEMENT: false        (Camera push/pull on Z-axis)
USE_TRUCK_MOVEMENT: false        (Camera lateral movement on X-axis)
USE_PEDESTAL_MOVEMENT: false     (Camera vertical movement on Y-axis)
USE_INFINITE_ZOOM: false         (Hypnotic continuous zoom effect)
USE_MORPH_TRANSITIONS: false     (Shape-to-shape transformations)
USE_ORGANIC_SHAPES: false        (Fluid, natural shape animations)
USE_MASK_TRANSITIONS: false      (Mask-based content reveals)
```

### **Camera Movement Parameters:**
```
DOLLY_SCALE_RANGE: 1.0-2.0       (How much to zoom in/out)
TRUCK_DISTANCE: 1920px           (How far to move laterally)
PEDESTAL_DISTANCE: 1080px        (How far to move vertically)
PARALLAX_MULTIPLIER: 0.5         (Background movement speed vs foreground)
```

### **Advanced Timing:**
```
MORPH_DURATION: 30               (Frames for shape morphing)
ZOOM_LOOP_DURATION: 60           (Frames per infinite zoom cycle)
MASK_REVEAL_DURATION: 30         (Frames for mask transitions)
CAMERA_MOVE_DURATION: 60         (Frames for camera movements)
```
